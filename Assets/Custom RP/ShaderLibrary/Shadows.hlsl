...
struct ShadowMask
{
    bool always; // indicate whether the shadow mask should always be used
    bool distance; // indicate whether distance shadow mask mode is enabled
    float4 shadows;
};
...
// support the case when there is a shadow mask but no realtime shadows
    ...
    OtherShadowData other, ShadowData global, Surface surfaceWS)
{
    ...
    float4 positionSTS = mul(
        ...
        float4(surfaceWS.position + normalBias, 1.0));
    return FilterOtherShadow(positionSTS.xyz / positionSTS.w, tileData.xyz);  // perspective projection
}
    ...
    OtherShadowData other, ShadowData global, Surface surfaceWS)
{
    ...
    float shadow;
    // global strength is used to determine whether we can skip sampling realtime shadows
