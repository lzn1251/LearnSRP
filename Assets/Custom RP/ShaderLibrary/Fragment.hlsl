...
Fragment GetFragment(float4 positionSS)
{
    ...
    f.screenUV = f.positionSS / _ScreenParams.xy;   // _ScreenParams.xy is screen pixel dimensions
...
float4 GetBufferColor(Fragment fragment, float2 uvOffset = float2(0.0, 0.0))
{
    ...
    return
       SAMPLE_TEXTURE2D_LOD(_CameraColorTexture, sampler_linear_clamp, uv, 0);
}
