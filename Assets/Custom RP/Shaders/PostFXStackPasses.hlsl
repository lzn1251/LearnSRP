...
{
    color = LinearToLMS(color);   // convert to LMS color space
...
{
    color = PositivePow(color, 1.0 / 2.2);   // convert to gamma space
    float t = saturate(Luminance(saturate(color), useACES) + _SplitToningShadows.w);   // limit the tint to their respective regions 
    float3 shadows = lerp(0.5, _SplitToningHighlights.rgb, 1.0 - t);
...
float3 ColorGradingHueShift (float3 color)
{
    ...
    color.x = RotateHue(hue, 0.0, 1.0);   // hue is defined on 0-1 color wheel
...
float3 ColorGrade (float3 color, bool useACES = false)
{
    ...
    color = max(color, 0.0);   // when contrast is increased this can lead to negative color components
...
float3 GetColorGradedLUT (float2 uv, bool useACES = false) {
    ...
    return ColorGrade(_ColorGradingLUTInLogC ? LogCToLinear(color) : color, useACES);     // use Log C space to support HDR
}
...
float3 ApplyColorGradingLUT (float3 color) {
    return ApplyLut2D(
        ...
        );    // interpret the 2D LUT strip as a 3D texture
}
