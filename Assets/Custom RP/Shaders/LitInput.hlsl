...
float3 GetNormalTS(InputConfig c)
{
    ...
    if (c.useDetail)
    {
        ...
        normal = BlendNormalRNM(normal, detail);     // blend original and detail normal; this function rotates the detail normal around the base normal
    }
...
float GetSmoothness(InputConfig c)
{
    ...
    if (c.useDetail)
    {
        float2 detail = GetDetail(c).b * INPUT_PROP(_DetailSmoothness);
        float mask = GetMask(c).b;
