...
public class Lighting {
	...
	private static string lightsPerObejctKeyword = "_LIGHTS_PER_OBJECT";
	...
	void SetupLights (bool useLightsPerObject, int renderingLayerMask)
	{
		NativeArray<int> indexMap = useLightsPerObject ? 
			cullingResults.GetLightIndexMap(Allocator.Temp) : default;  // retrieve the light index map
		...
		// eliminate the indices of all lights that aren't visible
		if (useLightsPerObject)
		{
			...
			indexMap.Dispose();
			Shader.EnableKeyword(lightsPerObejctKeyword);
		}
		else
		{
			Shader.DisableKeyword(lightsPerObejctKeyword);
		}
	...
	void SetupPointLight(int index, int visibleIndex, ref VisibleLight visibleLight, Light light)
	{
		...
		otherLightSpotAngles[index] = new Vector4(0f, 1f);   // make sure that point light aren't affected by spot angle
	...
	void SetupSpotLight(int index, int visibleIndex, ref VisibleLight visibleLight, Light light)
	{
		...
		float innerCos = Mathf.Cos(Mathf.Deg2Rad * 0.5f * light.innerSpotAngle);    // innerSpotAngle controls when the light starts attenuating
